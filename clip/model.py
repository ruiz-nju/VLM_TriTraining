...
def build_model(state_dict: dict, design_details):
    # 检查 state_dict 中是否包含 "visual.proj" 以判断是否使用视觉Transformer（ViT）架构
    vit = "visual.proj" in state_dict

    if vit:
        # 获取视觉模型的宽度，即第一层卷积的输出通道数
        vision_width = state_dict["visual.conv1.weight"].shape[0]

        # 计算视觉Transformer的层数，通过统计以 "visual." 开头且以 ".attn.in_proj_weight" 结尾的键的数量
        vision_layers = len(
            [
                k
        ...
        # 获取视觉模型的补丁大小，即第一层卷积的核尺寸
        vision_patch_size = state_dict["visual.conv1.weight"].shape[-1]

        # 计算网格大小，基于位置嵌入的数量减1后开平方并取整
        grid_size = round(
        ...
        # 获取视觉模型的宽度，即ResNet第一层第一个卷积层的输出通道数
        vision_width = state_dict["visual.layer1.0.conv1.weight"].shape[0]

        # 计算输出宽度，基于注意力池化位置嵌入的数量减1后开平方并取整
        output_width = round(
            (state_dict["visual.attnpool.positional_embedding"].shape[0] - 1) ** 0.5
        )

        # 由于不是ViT架构，补丁大小设置为None
        vision_patch_size = None

        # 确认位置嵌入的数量与输出宽度的平方加1相等，确保位置嵌入的正确性
        assert (
            output_width**2 + 1
            == state_dict["visual.attnpool.positional_embedding"].shape[0]
        )

        # 计算图像分辨率，通常为输出宽度乘以ResNet的下采样因子（例如32）
    ...
    # 获取Transformer的宽度，即最终LayerNorm层的特征维度
    transformer_width = state_dict["ln_final.weight"].shape[0]

    # 计算Transformer的头数，假设每个头的维度为64
    transformer_heads = transformer_width // 64

    # 计算Transformer的层数，通过统计以 "transformer.resblocks" 开头的唯一层数
    ...
    # 从state_dict中删除一些不需要的键，以避免加载时出错
    for key in ["input_resolution", "context_length", "vocab_size"]:
        if key in state_dict:
            del state_dict[key]

    # 转换模型权重，通常用于调整权重格式或进行预处理
    convert_weights(model)

    try:
        # 尝试将state_dict中的权重加载到模型中
        model.load_state_dict(state_dict)
    except:
        # 如果严格模式下加载失败，使用非严格模式加载，并打印缺失的键
